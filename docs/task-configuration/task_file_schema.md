...
| Field | Description |
| --- | --- |
| `seed` | The run's root seed, shared by scene randomization, waypoint randomization, and camera noise. Any integer — **including `0`** — is used verbatim; leaving it unset (`null`) keeps the run random but resolves one concrete seed that is logged, so `task.seed=<that value>` replays the run. |
...
keypoint boundaries; its `continuous: true` mode additionally collects the
interval without skipping segments between keypoints and marks each
configured keypoint boundary per step. See
