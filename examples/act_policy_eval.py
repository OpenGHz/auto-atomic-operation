...
def _matrix_to_quat_xyzw_batch(matrix: Any) -> np.ndarray:
    """和训练侧 act_relative_processor.matrix_to_quat_xyzw 完全一致的 numpy 版。"""
    m = np.asarray(matrix, dtype=np.float64).reshape(-1, 3, 3)
    m00, m11, m22 = m[:, 0, 0], m[:, 1, 1], m[:, 2, 2]
    qw = 0.5 * np.sqrt(np.clip(1.0 + m00 + m11 + m22, 0.0, None))
    qx = 0.5 * np.sqrt(np.clip(1.0 + m00 - m11 - m22, 0.0, None))
    qy = 0.5 * np.sqrt(np.clip(1.0 - m00 + m11 - m22, 0.0, None))
    qz = 0.5 * np.sqrt(np.clip(1.0 - m00 - m11 + m22, 0.0, None))
    qx = np.copysign(qx, m[:, 2, 1] - m[:, 1, 2])
    qy = np.copysign(qy, m[:, 0, 2] - m[:, 2, 0])
    qz = np.copysign(qz, m[:, 1, 0] - m[:, 0, 1])
    quat = np.stack((qx, qy, qz, qw), axis=-1)
    return _normalize_quat_batch_xyzw(quat)


def _rot6d_to_quat_xyzw_batch(rot6d: Any) -> np.ndarray:
    """rot6d -> quat(xyzw),列约定与训练一致(b1,b2,b3 为列)。"""
    return _matrix_to_quat_xyzw_batch(_rot6d_to_matrix_batch(rot6d))
...
class ACTPolicyAdapter:
    ...
    def __init__(
        ...
        self._validate_flags_against_checkpoint()

    def _checkpoint_uses_chunk_relative(self) -> bool:
        """从 policy_preprocessor.json 检测 checkpoint 是否内嵌 chunk-relative processor。

        训练时 ACTChunkRelativePoseProcessorStep 会作为 preprocessor step 存进 checkpoint,
        其 registry 名是 'airdc_act_chunk_relative_pose',class 路径含
        'ACTChunkRelativePoseProcessorStep'。据此判断该模型是否 chunk-delta 训练。
        """
        cfg_path = Path(self.checkpoint) / "policy_preprocessor.json"
        ...
        try:
            ...
            return False
        return (
            "airdc_act_chunk_relative_pose" in text
            or "ACTChunkRelativePoseProcessorStep" in text
        )

    def _validate_flags_against_checkpoint(self) -> None:
        """核对 CLI flag 与 checkpoint 真实训练表示,不符直接报错,杜绝'能跑但解释错'。

        - chunk-delta checkpoint 必须开 --delta-position-action + --delta-orientation-action,
          且不能开 --episode0-rela-pose;
        - 非 chunk-delta checkpoint 不能开 --delta-* (那样会把绝对/episode0 动作误当增量)。
        """
        is_chunk = self._checkpoint_uses_chunk_relative()
        delta_on = self.delta_position_action or self.delta_orientation_action
        if is_chunk:
            if self.episode0_rela_pose:
                raise ValueError(
                    "Checkpoint 是 chunk-delta 训练(内嵌 ACTChunkRelativePoseProcessorStep),"
                    "不能用 --episode0-rela-pose。请改用 "
                    "--delta-position-action --delta-orientation-action。"
                )
            if not (self.delta_position_action and self.delta_orientation_action):
                raise ValueError(
                    "Checkpoint 是 chunk-delta 训练,但未同时开启 "
                    "--delta-position-action 和 --delta-orientation-action。"
                    "缺失会把相对增量误当绝对位姿执行(能跑但动作全错)。"
                )
        else:
            if delta_on:
                raise ValueError(
                    "Checkpoint 不是 chunk-delta 训练(preprocessor 中无 "
                    "ACTChunkRelativePoseProcessorStep),但传了 --delta-position-action/"
                    "--delta-orientation-action。对绝对或 episode0 模型这会把绝对位姿误当增量。"
                    "请去掉 --delta-* (episode0 模型用 --episode0-rela-pose,绝对模型不带相对 flag)。"
                )
    ...
    def _rot6d_anchor_quat(self, observation: dict) -> np.ndarray:
        """chunk-delta 训练锚点:q0 = rot6d_to_quat(state 的 rot6d)。
        必须和 act_relative_processor 训练侧一致——数据里 arm/pose/orientation
        与 arm/pose/rotation_6d 约定不同(y轴翻转 180°),直接用 orientation 当锚会让
        姿态还原偏 180°。这里内联同一套 rot6d->quat 数学,避免 import 那个模块触发
        ProcessorStepRegistry 重复注册。"""
        rot6d = np.asarray(observation["arm/pose/rotation_6d"]["data"], dtype=np.float32)
        q = _rot6d_to_quat_xyzw_batch(rot6d)
        return _normalize_quat_batch_xyzw(q).astype(np.float32)
    ...
    def _state_parts(self, observation: dict) -> list[np.ndarray]:
        ...
        # episode0 相对语义固定作用于 (position, rotation_6d, gripper) 这三段;
        # 用 STATE_KEYS 取值以便与顶部配置保持单一来源,避免硬编码 key 漂移。
        pos = np.asarray(observation[STATE_KEYS[0]]["data"], dtype=np.float32)
        rot6d = np.asarray(observation[STATE_KEYS[1]]["data"], dtype=np.float32)
        grip = np.asarray(observation[STATE_KEYS[2]]["data"], dtype=np.float32)
    ...
    def check_io(self, observation: dict) -> None:
        ...
        if self.episode0_rela_pose:
            ...
            rel_quat = _normalize_quat_batch_xyzw(quat)
            # 和主 rollout 路径一致:chunk-delta 姿态锚点用 rot6d 转,不是 arm/pose/orientation
            anchor_quat = self._rot6d_anchor_quat(observation)
    ...
    def act(
        ...
        if need_chunk_anchor and (queue_empty or self._delta_anchor_pos is None):
            ...
            # chunk-delta 姿态锚点必须和训练一致:用 rot6d 转出的 q0,不是 arm/pose/orientation
            self._delta_anchor_quat = self._rot6d_anchor_quat(observation)
        ...
        if self.delta_orientation_action:
            delta_quat = _normalize_quat_batch_xyzw(quat)
            anchor_quat = (
                self._rot6d_anchor_quat(observation)
